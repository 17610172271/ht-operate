                ...
                <div class="page-contaoner">
                    <div class="lk-table m-t-sm">
                        <ul class="table-thead clear">
                            ...
                            <li class="col-xs-1 p-n" v-show="selectVal.indexOf('NAS编号')!=-1">NAS编号</li>
                        ...
                        <ul class="table-tbody clear" v-for="(item, index) in data.items" @click="selectItem(item.id)">
                            ...
                            <li class="col-xs-1 p-n over-omit" v-show="selectVal.indexOf('NAS编号')!=-1" :title="item.code">{{item.code}}</li>
                            <li class="col-xs-1 p-n over-omit" v-show="selectVal.indexOf('NAS名称')!=-1" :title="item.name">{{item.name}}</li>
                    ...
                    <el-dialog
                        ...
                        <div v-loading="modalLoading">
                            ...
                            <div class="clear m-b-sm">
                                <div class="col-xs-12 col-md-2 p-v-sm p-r-n text-right min-width-105"><span class="text-red">*</span>NAS名称:</div>
                                <div class="col-xs-12 col-md-8">
                                    <el-input placeholder="请输入NAS名称" :class="{'border-red': gatewayError}" v-model="detailVal.name" @blur="validateName(detailVal.name)"></el-input>
                                    <p v-if="gatewayError" class="text-red"><span class="fa fa-close m-r-xs"></span>请输入NAS名称</p>
                                </div>
                            </div>
                            <div class="clear m-b-sm">
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        data: () => ({
            ...
            loading: false,
            modalLoading: false,
            selectVal: ['ID', 'NAS编号', 'NAS名称', 'MAC地址','创建时间', '影院名称', '区域代理', '代理商', '操作'],
            showList: ['ID', 'NAS编号', 'NAS名称', 'MAC地址','创建时间','影院名称', '区域代理', '代理商', '操作'],
        ...
        methods: {
            ...
            getList () {
                this.loading = true
                this.$http.post(api.nas.nasList, {
                    ip: this.searchOptions[0].value,
                    name: this.searchOptions[1].value,
                    mac_address: this.searchOptions[2].value,
                    page: this.page,
                    limit: this.limit
                }).then(res => {
                    this.selectedGroup = []
            ...
            getDetail (item) {
                ...
                this.$http.get(api.nas.nasDetail, {
                    params: {
                        id: item.id
                    }
                }).then(res => {
            ...
            delItem(id) {
                if (id.length > 0) {
                    this.$confirm(id.length > 1 ? '此操作将批量删除选中NAS服务器, 是否继续?' : '此操作将删除该NAS服务器, 是否继续?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                        this.$http.get(api.nas.nasDel, {
                            params: {
                                id: id.join(',')
                            }
                        }).then(res => {
            ...
            dailogSubmit () {
                ...
                if (this.type === 'edit') {
                    this.$http.post(api.nas.nasEdit, this.detailVal).then(res => {
                        if (res.data.code === 1) {
                            this.$message({
                                type: 'success',
                                message: '修改成功'
                    ...
                    this.$http.post(api.nas.nasAdd, this.detailVal).then(res => {
                        if (res.data.code === 1) {
                            this.$message({
                                type: 'success',
                                message: '添加成功'
        ...
        watch: {
            ...
            page (val) {
                this.$router.replace({name: 'control_nas', query: {page: val}})
                this.getList()
            },
            limit (val) {
                this.getList()
